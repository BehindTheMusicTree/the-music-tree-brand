...
export function parseOrgSiteHref(value: string | undefined): string {
  ...
  if (!raw) {
    throw new Error(
      `Missing organization site URL: set environment variable ${ORG_DOMAIN_KEY} when building @behindthemusictree/brand (e.g. map GitHub repository variable DOMAIN_NAME into ${ORG_DOMAIN_KEY} in the publish workflow).`,
    );
  }
